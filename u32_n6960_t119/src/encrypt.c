...
int encrypt(unsigned char *s, const unsigned char *pk, const unsigned char *e)
{
...
#ifdef KAT
  {
	...
	ee.v8[SYS_N/8 - SYND_BYTES] = e[SYS_N/8-1]>>3;
        ...
        uint32_t dest[((NUM_ROW_PK+7)>>3)<<3] = {0};
        uint32_t chunk[SIZE_CHUNK_PK/4];

        uint8_t hash_32b[32];

        //uint32_t token = load4( pk );
        //pk += 4;
        ...
        for(int i=0;i<NUM_CHUNK_PK;i++) {
                uint32_t src =    ee.v8[i*4+0];
                src |= ((uint32_t)ee.v8[i*4+1])<<8;
                src |= ((uint32_t)ee.v8[i*4+2])<<16;
                src |= ((uint32_t)ee.v8[i*4+3])<<24;
