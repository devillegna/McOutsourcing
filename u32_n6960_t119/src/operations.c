...
)
{
	...
	if( encrypt((uint8_t*)_buf_c, pk, e) ) return -1;
...
#if 0
uint32_t rr_perm_u32[(1<<GFBITS)];
memcpy( rr_perm_u32 , r_perm_u32 , sizeof(rr_perm_u32) );
#endif
...
// public key
		...
		uint64_t pivot;
		// generate public key
		//if (pk_gen(pk, sk_irr_poly, rr_perm_u32, &pivot )) continue;
if (pk_gen(pk, sk_irr_poly, rr_perm_u32, &pivot , sk_cond_bytes , alpha_bs )) continue;
		}
#else
		// generate public key
