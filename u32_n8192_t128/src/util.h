...
static inline void u16x32_to_bs32( uint32_t *out, const uint16_t * u16vec , unsigned w_u16ele )
{
	uint32_t mat[16];
	uint16_t *mat16 = (uint16_t*)&mat[0];
	for(int i=0;i<32;i++) mat16[i] = u16vec[i];

	transpose_16x16( mat16 , mat16 );
	transpose_16x16( (mat16+16) , (mat16+16) );

	bs16_to_bs32( out , mat16 , mat16+16 , w_u16ele );
}

static inline void bs32_to_u16x32( uint16_t *u16vec, const uint32_t * in , unsigned w_u16ele )
{
	uint16_t *mat16 = u16vec;
	bs32_to_bs16(mat16, mat16+16, in, w_u16ele );
	for(int i=w_u16ele;i<16;i++) mat16[i] = 0;
	for(int i=w_u16ele;i<16;i++) mat16[16+i] = 0;

	transpose_16x16( mat16 , mat16 );
	transpose_16x16( (mat16+16) , (mat16+16) );
}

static inline void bs32_to_gf( uint16_t *out, const uint32_t * in )  { bs32_to_u16x32( out , in , GFBITS ); }

static inline void gf_to_bs32( uint32_t *out, const uint16_t * gfs )  { u16x32_to_bs32( out , gfs , GFBITS ); }
