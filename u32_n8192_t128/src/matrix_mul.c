...
void matrix_mul2_32( uint32_t *mat_c , const uint32_t* mat_a , int h_a, int w_a, const uint32_t *mat_b , int w_b )
{
  for(int i=0;i<h_a;i++) for(int j=0;j<w_b;j++) mat_c[i*w_b+j] = 0;
...
void matrix_mul2( uint32_t *mat_c , const uint32_t* mat_a , int h_a, int w_a , const uint32_t *mat_b , int w_b )
{
	for(int i=0;i<h_a;i++) {
		const uint32_t *row_a = mat_a+w_a*i;
		uint32_t mask = row_a[0];
