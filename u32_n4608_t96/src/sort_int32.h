...
#define uint32_MINMAX_u16payload(a,b,pa,pb) \
do { \
  uint32_t c = ((uint64_t)b - (uint64_t)a)>>32; \
  uint16_t pc = c&0xffff; \
  c &= a ^ b; \
  pc &= pa ^ pb; \
  a ^= c; \
  pa ^= pc; \
...
static inline void uint32_sort_u16payload(uint32_t *x,uint16_t *y, long long n)
{
  long long top,p,q,r,i;
  ...
  for (p = top;p > 0;p >>= 1) {
    for (i = 0;i < n - p;++i)
      if (!(i & p))
        uint32_MINMAX_u16payload(x[i],x[i+p],y[i],y[i+p]);
    i = 0;
    for (q = top;q > p;q >>= 1) {
      for (;i < n - q;++i) {
        if (!(i & p)) {
          ...
          uint16_t ya = y[i + p];
          for (r = q;r > p;r >>= 1)
            uint32_MINMAX_u16payload(a,x[i+r],ya,y[i+r]);
          x[i + p] = a;
          y[i + p] = ya;
        }
      }
