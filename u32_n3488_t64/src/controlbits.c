...
unsigned controlbitsfromweight(unsigned char * controlbits, uint32_t * weight,long long w,long long n)
{
    {
    uint16_t pi[n];
    ...
    uint16_t *out_pi = (uint16_t*) weight;
    for (int i=0; i<n;i++) out_pi[i] = pi[i];
    }

    controlbitsfrompermutation(controlbits, (int16_t*)weight, w, n);
    return 0;
}
