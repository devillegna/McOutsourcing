...
)
{
	...
	// SYS_N/8=3488/8=436; (1<<GFBITS)*4=16384; SYS_T=64
	...
	// sk: [ sk_seed:32 + pivot:8 + irr_poly:2*SYS_t + COND_BYTES + SYS_N/8 ]  // COND_BYTES:5888 , SYS_N/8:436
...
// secret key
		...
		for (int i = 0; i < (1 << GFBITS); i++) { r_perm_u32[i] = load4( r_perm+i*4 ); }  // 16KB
		...
		if( 0 != controlbitsfromweight(sk_cond_bytes, r_perm_u32, GFBITS, 1 << GFBITS) ) continue;
		uint16_t *alpha = (uint16_t*)r_perm_u32;
...
// public key
		// generate public key
		if (pk_gen(pk, sk_irr_poly, r_next_seed , sk_cond_bytes , alpha)) continue;

		break;
	}
	return (key_gen_fail<100)? 0 : -1;
}
